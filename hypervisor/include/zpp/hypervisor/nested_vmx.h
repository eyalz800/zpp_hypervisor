...
/**
 * `window_on_tpr` with the two defects that made it deliver nothing
 * fixed, and nothing else changed.
 *
 * Measured on one processor of one boot, and this is the reading the
 * whole switch exists to move:
 *
 *     ZPP_WINDOW_ON_TPR=0   0x2f delivered  9,627   clock 388,241
 *     ZPP_WINDOW_ON_TPR=1   0x2f delivered     11   clock   3,904
 *
 * So the wasteful window storm is how the dispatch vector was actually
 * being delivered, and replacing it with the TPR threshold stopped
 * delivery outright. Two things in that path never complete a delivery,
 * and each is a place the request is *dropped* rather than held:
 *
 * - **The drop is noted and nothing is armed at the moment it
 *   happens.** `on_l2_exit`'s `tpr_below_threshold` case sets
 *   `window_armed_on_drop` and returns `handled`, and an exit this VMM
 *   handles is resumed straight into the second-level guest without
 *   going through `build_vmcs02`. vmcs02 at that instant has no
 *   interrupt window - it was stripped when the priority was up - and
 *   the threshold has just been taken back down. So the one moment the
 *   vector could be taken is the one moment nothing in the machine will
 *   cause an exit at which the level above could take it, and the
 *   guest runs on with the deferred call still queued.
 *
 *   KVM does the opposite and does it in one line: `vmx_enable_irq_
 *   window` (`.references/kvm/vmx.c:4937`) sets `CPU_BASED_INTR_WINDOW_
 *   EXITING` in the **live** VMCS, and `kvm_check_and_inject_events`
 *   (`.references/kvm/x86.c:10445`) re-arms it on every entry while
 *   `kvm_cpu_has_injectable_intr` is still true. The vector stays in
 *   the IRR until something actually takes it - `kvm_apic_has_
 *   interrupt` only *reads* it (`.references/kvm/lapic.c:2934`) - so a
 *   failed attempt costs a retry and never a request.
 *
 * - **The strip and the arm are decided from two different samples of
 *   the priority.** The window is stripped against
 *   `l2_entry_priority`, which is the byte read at the *previous*
 *   entry, and the threshold is armed against `entry_vtpr`, read from
 *   the virtual-APIC page a hundred lines later in the same call. When
 *   the priority fell in between - which is precisely the transition
 *   being waited for - the first says "blocked, strip the window" and
 *   the second says "already below the threshold, do not arm it"
 *   (`window_threshold_withheld`). The entry is then made at a
 *   priority that admits the vector with neither mechanism in place.
 *
 * On, both decisions are taken from the one fresh read, so they cannot
 * disagree, and the drop exit puts the interrupt window into vmcs02
 * before resuming. The threshold is still armed only where the level
 * above set the TPR shadow and left the threshold at zero, and it is
 * still armed only over a priority that is at or above the class - so
 * SDM 29.2.1.1's "bits 3:0 of the TPR threshold should not be greater
 * than bits 7:4 of VTPR" (`.references/sdm.txt:202124`) holds by
 * construction rather than by a second test.
 *
 * The class is the dispatch vector's, 2, the same quantity KVM writes
 * in `vmx_update_cr8_intercept` (`.references/kvm/vmx.c:6728`,
 * `tpr_threshold = (irr == -1 || tpr < irr) ? 0 : irr` with `irr`
 * already shifted down by four in `update_cr8_intercept`,
 * `.references/kvm/x86.c:10193`).
 *
 * **Off by default**, so an A/B against it is one variable. It is
 * independent of `window_on_tpr` rather than a modifier of it: turning
 * both on is refused by a static assertion, because they write the same
 * per-processor state to opposite ends.
 */
