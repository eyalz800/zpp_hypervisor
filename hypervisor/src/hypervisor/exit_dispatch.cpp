...
#define ZPP_SLOW_EXITS 0
    ...
    case basic_reason::vmcall: {
        ...
        // **`HvCallAddLogicalProcessor`, and only that call.**
        //
        // Measured on every two-processor boot: cpu 0's last exit before
        // the partition resets is `vmcall` from the level above with
        // hypercall code **0x76**, and the reset follows immediately.
        // The application processor itself is fine by then - the
        // operating system's own start-up IPI reached it, this VMM
        // applied `vector 0x2`, and `last_resume_rip` shows it was
        // resumed into the trampoline at `0x2000`, whose GDT is valid.
        // What fails is the *registration* of that processor with the
        // guest hypervisor, which is what this call is for
        // (`hypercall_table.csv`: "input {LpIndex, ApicId,
        // ProximityDomainInfo}").
        //
        // The answer below is `#UD`, which is right for the twelve VMX
        // instructions and wrong for a hypercall. Answering *all*
        // hypercalls with a status instead was tried and is a **100x
        // regression** (898cab6): the guest died at 5,354 exits against
        // 561,912, because `#UD` reads as "no hypercall interface" and
        // the guest hypervisor falls back, while a status says "an
        // interface is here and refuses you". So the refusal kind is
        // load-bearing and must not be changed wholesale.
        //
        // This answers one code, successfully, and leaves every other
        // hypercall taking `#UD` exactly as before. Success is the
        // truthful answer rather than a convenient one: the logical
        // processor really is running, started by the architectural
        // INIT-SIPI-SIPI path this VMM already virtualises, so "it has
        // been added" is a statement about the machine and not a lie
        // told to get past a check.
        //
        // Result convention: RAX carries the status in its low sixteen
        // bits, 0 being HV_STATUS_SUCCESS, and RDX is cleared. RIP is
        // advanced past the `vmcall`, which is what a retired
        // instruction requires - unlike the `#UD` path below, which
        // deliberately leaves it in place.
        if (auto slot = cpuid; slot < max_cpus) {
            constexpr std::uint64_t add_logical_processor = 0x76;
            constexpr std::uint64_t hypercall_code_mask = 0xffff;

            if (!this->running_l2[slot] &&
                (basic_reason::vmcall == reason) &&
                (add_logical_processor ==
                 (context.rcx & hypercall_code_mask))) {
                this->add_logical_processor_answered =
                    this->add_logical_processor_answered + 1;

                log("cpu {} answered HvCallAddLogicalProcessor, "
                    "answer {}",
                    cpuid,
                    this->add_logical_processor_answered);
                ...
                // `advance_rip` is the local default and stays true, so
                // breaking here retires the `vmcall` - which is what an
                // answered hypercall requires. The `#UD` path below
                // clears it instead, deliberately, because a fault
                // leaves RIP on the faulting instruction.
                break;
            }
        }
