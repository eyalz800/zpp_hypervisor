        ...
        if ((cpu < max_cpus) && this->evmcs_active[cpu]) {
            ...
            if (second_level) {
                ...
                *reinterpret_cast<volatile std::uint8_t *>(
                ...
                // **Dirty the whole page on every entry, even if nothing
                // is written into it.** The layer below marks every group
                // clean once it has read them, and then re-reads nothing
                // until a group is marked dirty again. A real VMCS does
                // not work that way - a field written once stays written
                // - and `write_vmcs02_control` relies on exactly that: it
                // keeps its own cache and *skips* a control whose value
                // has not changed.
                //
                // Those two are individually right and together wrong. On
                // the second entry every control matched its cached value,
                // so every write was skipped, so nothing cleared the clean
                // mask, so the layer below re-read none of the fields and
                // ran the entry against what it had already consumed.
                // Measured: `hv_clean_fields` reading `0xffff` at a
                // `VMfailValid`, with the page's contents themselves
                // perfectly good.
                //
                // Clearing it here costs one store an entry and makes the
                // skip harmless again: the values are still in the page,
                // and this says "read them".
                *reinterpret_cast<volatile std::uint32_t *>(
                    this->evmcs[cpu] +
                    arch::x86_64::vmx::evmcs_clean_fields_offset) =
                    arch::x86_64::vmx::evmcs_all_dirty;
