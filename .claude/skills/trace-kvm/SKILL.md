...
What is left untested is whether this kernel's ftrace records at all.
`available_tracers` lists only `nop`, and no producer has ever been seen
to arrive. Check that with a **bounded** reader before anything else,
and if a `trace_marker` write does not come through in five seconds,
stop - the mechanism is broken and no amount of arming will fix it.
