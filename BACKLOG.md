...
## Item 2, second attempt: the condition is *whose* state vmcs02 holds

The first attempt reset the guest **218 times**. The bug is worth
stating precisely because the analysis behind it was right and the
conclusion drawn from it was not.

SDM 30.3.1 says the processor saves every one of these fields into the
guest-state area on every VM exit. True, checked, cited. What it
licenses is *"after an exit from a guest, vmcs02 holds that guest's
state"* - and what the code did was skip the write-back **always**,
which is a much stronger claim. It is false twice over:

- **the first entry to a vmcs02**, where nothing has been saved and the
  region holds what it was cleared to. The guest was launched with a
  zeroed guest state, which is exactly the reset loop observed;
- **whenever the level above VMPTRLDs a different vmcs12**, since vmcs02
  is reused per processor and its contents then belong to a *different*
  second-level guest.

`vmcs02_launched` and `guest_current_vmcs` already track both halves.
Neither was consulted. `may_defer_guest_state` now requires both, and
`forget_vmcs02_contents` clears the deferral because it invalidates
exactly the contents the deferral describes.

One more correction fell out of it: when deferral is **not** licensed,
the deferred fields are written **unconditionally** rather than merely
un-elided. `guest_state_cache` is stale for those indices by
construction - nothing refreshed it while they were being deferred - so
comparing against it could skip a write that is owed. That is the same
shape of error as the original, one level down.

Three cases in `nested_exit` assert the licensing directly: a vmcs02
that has never run may not defer, a launched one whose last exit was
this guest's may, and a different vmcs12 may not. 1,089 checks.

**What this cost and what it is worth recording for.** The measurement
was right, the citation was right, the field partition was right, the
exclusion of the two shadowed fields was right - and the change still
reset the guest 218 times, because the safety argument answered *what*
may be deferred and never asked *when*. **A citation that licenses a
narrow claim does not license the broad one built on it**, and the gap
between the two is where this went.

The guard that caught it was cheap and immediate: exit counters running
backwards between two reads is the tell for a reloading hypervisor, and
it showed up on the first pair of dumps.
