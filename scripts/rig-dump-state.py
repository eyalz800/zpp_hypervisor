...
def dump_dropped_requests(args, elf, instance):
    ...
    members = ["pending_vector_asked", "pending_vector_coalesced",
               ...
               "window_armed_at_drop", "window_armed_at_drop_refused",
               "window_threshold_refused"]
    off = gdb_offsets(elf, members)
    ...
    for cpu in range(args.cpus):
        ...
        armed_refused = word("window_armed_at_drop_refused", cpu)
        threshold_refused = word("window_threshold_refused", cpu)

        print(f"  window withheld {deferred:,}, priority drops "
              f"reported {granted:,}, window handed back in vmcs02 "
              f"{armed:,}")

        if deferred and not granted:
            print("  *** DEADLOCK SHAPE: the window was withheld and "
                  "the processor never reported a drop. The threshold "
                  "is not being armed, or the exit is being reflected "
                  "away - see window_threshold_armed. ***")
        if granted and not armed:
            print("  *** the drop was reported and the window was NOT "
                  "handed back. That is exactly ZPP_WINDOW_ON_TPR's "
                  "defect: the drop is noted and the guest resumes "
                  "with nothing armed. ***")
        if armed_refused:
            print(f"  {armed_refused:,} drops where vmcs12 had stopped "
                  f"asking for a window, so none was given")
        if threshold_refused:
            print(f"  *** {threshold_refused:,} withholdings with no "
                  f"threshold to replace them - the TPR shadow was "
                  f"absent or the level above had set its own. Those "
                  f"are windows removed and not replaced. ***")
