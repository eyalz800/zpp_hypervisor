...
ifneq ($(ZPP_TARGET_TYPES), )
build:
	@for target_abi in $(ZPP_TARGET_TYPES); do \
		$(MAKE) -s -f $(ZPP_THIS_MAKEFILE) ZPP_TARGET_TYPES= ZPP_TARGET_TYPE=$$target_abi; \
	done
clean:
	@for target_abi in $(ZPP_TARGET_TYPES); do \
		$(MAKE) -s -f $(ZPP_THIS_MAKEFILE) clean ZPP_TARGET_TYPES= ZPP_TARGET_TYPE=$$target_abi; \
	done
rebuild:
	@for target_abi in $(ZPP_TARGET_TYPES); do \
		$(MAKE) -s -f $(ZPP_THIS_MAKEFILE) rebuild ZPP_TARGET_TYPES= ZPP_TARGET_TYPE=$$target_abi; \
	done
else
build: build_single
clean: clean_single
